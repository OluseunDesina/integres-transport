...
describe('Login', () => {
  ...
  it('navigates to /record on successful login', async () => {
    const router = TestBed.inject(Router);
    const navigateSpy = spyOn(router, 'navigate').and.resolveTo(true);
    authApi.login.and.resolveTo({ ok: true });
    component['form'].setValue({ email: 'staff@example.com', password: 'correct-horse' });

    await component['onSubmit']();

    expect(navigateSpy).toHaveBeenCalledWith(['/record']);
  });
});

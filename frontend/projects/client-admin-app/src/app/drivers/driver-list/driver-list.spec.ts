...
describe('DriverList', () => {
  ...
  it('shows a warning pill listing the compliance_warnings count', () => {
    store.items.set([makeDriver({ compliance_warnings: ["Driver's license expired on 2026-01-01"] })]);
    fixture.detectChanges();

    const cells = fixture.debugElement.queryAll(By.css('tbody td'));
    expect(cells.some((c) => (c.nativeElement.textContent as string).includes('1 warning(s)'))).toBe(
      true
    );
  });
});

...
export class MyBookings implements OnInit {
   ...
   * would be a worse screen, not a tidier one.
   *
   * No "Track this trip" or "Report a problem" here, unlike
   * `customer-app`'s own copy of this screen — neither `trip-tracking`
   * nor `report-issue` exist in this app (docs/specs/22-marketplace.md's
   * own non-goals); those routes would 404.
   */
  ...
  protected async payNow(booking: Booking): Promise<ConfirmDialogResult> {
    ...
    // No `use_wallet_balance` here, unlike `customer-app`'s own copy of
    // this screen: the marketplace payment endpoint is Paystack-only
    // for this slice (docs/specs/22-marketplace.md's own non-goal) — the
    // wallet-balance checkbox below never actually renders for a
    // marketplace passenger anyway, since `GET /wallet/mine/` 400s for a
    // Business under a Client that isn't theirs and this file's own
    // `loadWalletBalances` already treats that as a 0.00 balance.
    const { data, error } = await this.api.POST('/api/v1/marketplace/payments/', {
    ...
    // docs/specs/22-marketplace.md slice 2: a 409 here can mean the seat
    // hold lapsed moments before this click (`initiate_payment`'s own
    // `expire_stale_holds_for_booking` check, closing the race against
    // the once-a-minute sweep) — refetching is what turns this row's
    // status pill into "Expired" and drops the Pay button, which says
    // what happened far more plainly than any wording of the error text
    // could, and does so regardless of which of `BookingNotPayable`'s
    // several causes this particular 409 was.
    await this.store.getAll();

...
| `marketplace-app` | Passengers, cross-operator | **Spec 22, two slices.** A Wakanow/Trip.com-style aggregator: register/browse/search with no session at all, search across every operator on the platform (never grouped by one), reserve, and pay — same booking-flow screens `customer-app` uses, deliberately duplicated rather than shared for now. Restyled as a storefront by spec 24. Backed by a dedicated `apps.marketplace` Django app plus ADR-0009's cross-tenant read mechanism, not a widened `customer-app`. See spec 22 for both slices' own Implementation notes |
...
| **Spec 22 — Marketplace** | A cross-operator trip marketplace, a new `marketplace-app`, ADR-0009's cross-tenant read mechanism | **Complete, both slices** — see `docs/specs/22-marketplace.md`'s own two Implementation notes for the full account, including the deadlock and cache-eviction bugs slice 2's hold-expiry fix caught only by running the real test suites. Not part of the Transit OS adoption roadmap (§1's "Transit OS adoption roadmap is done" refers to specs 1–21 only) |

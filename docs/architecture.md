...
serving five Angular 20 frontends** from a single monorepo — four
white-labeled, plus `marketplace-app` (spec 22), which deliberately
carries no white-labeling of its own since it aggregates every operator
rather than representing one. A dedicated Flutter validator app is
still planned for a later phase (out of scope for everything built so
far); `validator-app` (Phase 4b, §7) is an installable Angular PWA
standing in for it in the meantime, not that future app itself.

The five frontends are not five products — they're five access
surfaces onto the same tenant-scoped backend:

| App | Who uses it | Purpose |
|---|---|---|
| `customer-app` | Passengers | **Rebuilt to the design system in spec 14 slice 5** — every screen including login, brand presence via `app-brand-mark`, a visible booking-flow step indicator, and the `consumer` surface profile actually reaching the controls (16px, so a focused input no longer makes iOS Safari zoom the page). Account, booking (trip search, seat picker, booking confirm, my-bookings — `docs/specs/4-fares-seating-booking-frontend.md`), payment (my-bookings' single Pay action → real Paystack checkout, Phase 5 frontend Slice A; the blended-payment revisit below folded the previously-separate "Pay from wallet" button into it, applying the wallet balance first and charging only the remainder, if any — and spec 14 slice 5 moved the whole thing behind a confirmation dialog carrying the amount, the wallet split as a `ui-toggle`, and a button naming the outcome, since one tap used to move money and the wallet option was a bare checkbox in a read table cell), a `credentials` screen to issue/view/revoke a Tap & Go `TapCredential` (Phase 4b frontend close-out), a ticket/QR view on paid bookings (Phase 6 frontend Slice A), and a `wallet` screen (business picker, balance, top-up) (Phase 7 Slice B) — see each phase's own spec implementation note. **2026-09-20**: three of `marketplace-app`'s own spec 22 slice 3 UI/UX pieces ported over — a seats-left count on search results, a sticky trip/booking summary sidebar on `seat-picker`, and a "Change seat" link plus hold timer on `booking-confirm` — see spec 22's own cross-app note for what shipped and what deliberately did not (this app keeps its manual seat map) |
...
| `marketplace-app` | Passengers, cross-operator | **Spec 22, three slices.** A Wakanow/Trip.com-style aggregator: register/browse/search with no session at all, search across every operator on the platform (never grouped by one), reserve, and pay — same booking-flow screens `customer-app` uses, deliberately duplicated rather than shared for now. Slice 3 removed seat choice from the booking flow entirely (server-side auto-assign plus a "Change seat" link). Restyled as a storefront by spec 24 (slice 1; slice 2's own scope shifted under it, see that spec). Backed by a dedicated `apps.marketplace` Django app plus ADR-0009's cross-tenant read mechanism, not a widened `customer-app`. See spec 22 for all three slices' own Implementation notes |
...
| **Spec 21 — Passenger experience** | Senior Mode, hold countdown, mobile navigation | **Slice 1 (responsive navigation) complete**, `docs/specs/21-passenger-experience.md`. `customer-app`'s `AppShell` moves its nav to a fixed bottom tab bar below Tailwind's own `sm` breakpoint (640px) — closing the 390px nav-overflow defect recorded since spec 10's own visual pass — and keeps the original top-bar row at `sm` and above, driven by one `BreakpointObserver` signal gating a single `@if` so exactly one `nav[aria-label="Primary"]` is ever in the DOM. A new `e2e/responsive-nav.ts` harness (per `responsive-tables.ts`'s own shape) caught a real 320px finding: a flex item's default `min-width: auto` held one two-word tab label to 28px wide — under the 44px touch-target minimum — while its one-word neighbours kept their full intrinsic width; fixed with `min-w-0` plus `break-words`. Running the rest of `customer-app`'s own Playwright suite (not just the new spec, since `AppShell` is chrome every screen mounts) surfaced two further pre-existing bugs never actually run to green before: a fixture helper reading a staff-gated endpoint with a passenger token, and `ui-map`'s zoom control, attribution link and markers staying keyboard-focusable inside the map's own `aria-hidden` container — both fixed (`keyboard: false` on the Leaflet map/markers, `tabindex="-1"` swept onto the remaining anchors, `text-decoration: underline` on the attribution link in both apps' `styles.css`). A third, unrelated contrast bug in the same component's marker popup was left open for this spec's own slice 3 or spec 20's follow-up. No backend change. 257 customer-app unit tests changed, 728 client-admin-app unit tests re-confirmed unaffected. **Slice 2 (hold countdown) complete**: `hold_expires_at`/`hold_expires_in_seconds` added to `BookingSerializer`, both `SerializerMethodField`s reading the same per-booking `SeatReservation` rows `seats` already batches (no new query anywhere), computed from the earliest **`HELD`** reservation only — not "any non-released row," so an already-paid booking (all `CONFIRMED`) gets `null` without a separate check. A new `CountdownClock` (`@shared-data`, `providedIn: 'root'`, one shared `setInterval` ref-counted to subscribers — a distinct primitive from `Poller`, not a reuse, since this is one tick source several component instances share rather than one screen's own polling need) and `ui-countdown` (`@shared-ui`, counts down from seconds alone, never `Date.now()`; emits `expired` and stops without asserting the hold is gone, so both consumers re-fetch instead). `booking-confirm` no longer navigates to `/my-bookings` the instant a submit succeeds — it shows a held/confirmed panel with the countdown first, since the spec's own reason for returning both fields on the create response ("so the confirm screen can start counting down without a second request") needs somewhere to render it; "Continue to My Bookings" replaces the automatic redirect. `my-bookings` gained the same countdown beside each `pending_payment` row's status pill, in both responsive tiers. A second spec claim corrected against the model (the first was slice 1's `validator-app` scope note): quick-book bookings hold a real seat (`create_reservation` runs for them exactly as for a manually-picked one) and get a real countdown, contrary to the spec's edge case grouping them with open seating, which holds nothing. 7 new backend tests (1178/1178, up from 1171); 1617 frontend unit tests (up from 1598). **Slice 3 (Senior Mode) complete, closing this spec and the whole Transit OS adoption roadmap**: `SeniorModeStore` (`customer-app`-local, persisted, sets `data-senior="true"` on `<html>`) plus a `theme.css` token block keyed off that attribute — `html[data-senior='true'] { font-size: 175% }` scales the whole `rem`-based type ramp, control/target sizes and spacing together from one rule, since everything in this workspace is already `rem`-based; contrast tokens raised to a measured 7:1 (AAA); a header `ui-toggle` reachable at every viewport; QR treatment enlarged on both ticket/credential screens. Four real findings from the visual pass, two fixed: `ui-page-header` silently clipped an overflowing title/description instead of wrapping (missing `min-w-0` on its flex wrappers left `break-words` nothing to break inside — fixed, and not Senior-Mode-specific, just first exposed by its larger type); an `sr-only` countdown announcement escaped `ui-table`'s scroll wrapper entirely and inflated page scrollWidth (the wrapper lacked `position: relative` to contain it, fixed alongside the same `min-w-0` flex-sizing bug). One is a Playwright `fullPage`-screenshot artifact misplacing a `position: fixed` bar (confirmed the live app renders correctly; no code change). One is a named, accepted gap: `ui-table`'s columns stay at a deliberately frozen "console density" (three prior iterations' own fix, documented in its own file) that `rem`-based root scaling cannot selectively respect, so a narrow column can wrap mid-word under Senior Mode — real, but `docs/specs/14-design-system-and-ui-rebuild.md` territory to actually fix. 1633 frontend unit tests (up from 1617); no backend change. Self-check catch-up for specs 19–21 still owed, now three specs deep |
| **Spec 22 — Marketplace** | A cross-operator trip marketplace, a new `marketplace-app`, ADR-0009's cross-tenant read mechanism | **Complete, three slices** — see `docs/specs/22-marketplace.md`'s own three Implementation notes for the full account, including the deadlock and cache-eviction bugs slice 2's hold-expiry fix caught, and slice 3's `.objects`-vs-`all_objects` bug caught while wiring up traveler read-back. Slice 3 removed seat choice from the booking flow (auto-assign plus "Change seat"), each caught only by running the real test suites. Not part of the Transit OS adoption roadmap (§1's "Transit OS adoption roadmap is done" refers to specs 1–21 only) |
| **Spec 23 — Super-admin Business/KYB merge** | Folding `super-admin-app`'s separate Business list and KYB review queue into one filterable list | **Complete** — see `docs/specs/23-super-admin-business-kyb-merge.md`'s own Implementation note, including the `BooleanField`-vs-`ChoiceField` bug its own test suite caught |
| **Spec 24 — Marketplace redesign** | `marketplace-app` reworked into a travel-aggregator storefront, baselined on a survey of Omio, Busbud, FlixBus, Rome2Rio, BuuPass and Wakanow: segmented search bar, recent searches, results with filters/sort tabs/Cheapest-Fastest badges/date strip, split-screen auth. Frontend only | **Slice 1 of 2 complete** (storefront: shell, landing, results, auth) — see `docs/specs/24-marketplace-redesign.md`'s Implementation note. Slice 2's own scope shifted under it once spec 22 slice 3 shipped (independently requested) — see that spec's own "Slice 2" bullet for the reconciliation note; it is now a visual restyle of the auto-assign/change-seat flow, not the seat-map flow it was written against |

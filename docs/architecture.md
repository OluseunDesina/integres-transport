...
**Status as of 2026-08-09**: Phases 0–3 complete and independently
self-checked. This document describes the system as it actually is
today, and separately, clearly, what is planned but not yet built. See
§7 for the full built-vs-planned breakdown, and §8 for known
...
|---|---|---|
| `customer-app` | Passengers | Account, booking (trip search, seat picker, booking confirm, my-bookings — `docs/specs/4-fares-seating-booking-frontend.md`), payment (my-bookings' "Pay now" → real Paystack checkout, Phase 5 frontend Slice A — see `docs/specs/5-payments-wallet-ledger.md`'s own implementation note), and (Phase 4b frontend close-out) a `credentials` screen to issue/view/revoke a Tap & Go `TapCredential` — see `docs/specs/4b-tap-and-go.md`'s own implementation note. No wallet screen yet |
